...
fn validation(req: Request) -> Result<Response> {
    if req.method() == http::Method::OPTIONS {
        ...
        res.headers_mut().insert("webhook-response-origin", origin.try_into()?);
        res.headers_mut().insert("webhook-response-rate", rate.try_into()?);
        res.headers_mut().insert("spin-component", "rust-outbound-http".try_into()?);
        Ok(res)
    } else {
